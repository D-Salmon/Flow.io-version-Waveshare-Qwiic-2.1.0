...
#ifndef FLOW_PROVISIONING_AP_PASSWORD
...
void WifiProvisioningModule::buildApCredentials_()
{
    ...
    snprintf(apPass_, sizeof(apPass_), "%s", configuredPassword);
    Serial.printf("\r\n[SECURITY] Flow.io provisioning AP: ssid=%s password=%s\r\n",
                  apSsid_,
                  apPass_);
}
...
bool WifiProvisioningModule::handleSaveRequest_(const char* query)
{
    ...
    const bool clearPassRequested =
        getQueryParam_(query, "clear_pass", clearPassStr, sizeof(clearPassStr)) &&
        (clearPassStr[0] == '1' ||
         strcasecmp(clearPassStr, "true") == 0 ||
         strcasecmp(clearPassStr, "on") == 0 ||
         strcasecmp(clearPassStr, "yes") == 0);
    ...
    wifi["ssid"] = portalSsid_;
    if (portalPass_[0] != '\0' || clearPassRequested) {
        wifi["pass"] = clearPassRequested ? "" : portalPass_;
    }
    const size_t patchLen = serializeJson(patch, portalJson_, sizeof(portalJson_));
    if (patchLen == 0U || patchLen >= sizeof(portalJson_)) {
        return false;
    }
