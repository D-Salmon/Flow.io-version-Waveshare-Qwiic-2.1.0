...
static bool verifyOtaSignature_(const uint8_t digest[32], const char* signatureBase64)
{
...
#ifndef FLOW_WEB_ADMIN_USER
#define FLOW_WEB_ADMIN_USER "admin"
#endif

#ifndef FLOW_WEB_ADMIN_PASSWORD
#define FLOW_WEB_ADMIN_PASSWORD ""
#endif
...
struct DiagnosticRelayOffTask {
    const CommandService* cmd = nullptr;
    uint8_t slot = 0;
    uint32_t delayMs = 0;
};

void diagnosticRelayOffTask_(void* arg)
{
    DiagnosticRelayOffTask* task = static_cast<DiagnosticRelayOffTask*>(arg);
    if (task) {
        vTaskDelay(pdMS_TO_TICKS(task->delayMs));
        if (task->cmd && task->cmd->execute) {
            char cmdArgs[64] = {0};
            char reply[96] = {0};
            snprintf(cmdArgs, sizeof(cmdArgs), "{\"slot\":%u,\"value\":false}", (unsigned)task->slot);
            (void)task->cmd->execute(task->cmd->ctx, "pooldevice.write", cmdArgs, nullptr, reply, sizeof(reply));
        }
        vPortFree(task);
    }
    vTaskDelete(nullptr);
...
uint8_t diagDsCrc8_(const uint8_t* data, uint8_t len)
{
    uint8_t crc = 0;
    while (len--) {
        uint8_t inbyte = *data++;
...
bool diagParseRom_(const char* text, uint8_t rom[8])
{
    ...
    for (const char* p = text; *p != '\0'; ++p) {
        ...
        if (highNibble < 0) {
            ...
            rom[count++] = (uint8_t)((highNibble << 4) | nibble);
            highNibble = -1;
        }
    }
    return count == 8U && highNibble < 0 && rom[0] == 0x28U && diagDsCrc8_(rom, 7U) == rom[7];
}

void diagPrintRom_(Print& out, const uint8_t rom[8])
{
    for (uint8_t i = 0; i < 8U; ++i) {
        if (i > 0U) out.print('-');
        out.printf("%02X", (unsigned)rom[i]);
    }
}

bool diagI2cWrite_(uint8_t addr, const uint8_t* data, uint8_t len)
{
...
bool diagI2cRead_(uint8_t addr, uint8_t* data, uint8_t len)
{
...
bool diagDs2484SetPointer_(uint8_t addr, uint8_t pointer)
{
    const uint8_t cmd[2] = {0xE1, pointer};
    return diagI2cWrite_(addr, cmd, sizeof(cmd));
}

bool diagDs2484ReadStatus_(uint8_t addr, uint8_t& status)
{
    if (!diagDs2484SetPointer_(addr, 0xF0)) return false;
    return diagI2cRead_(addr, &status, 1);
}

bool diagDs2484WaitReady_(uint8_t addr, uint32_t timeoutMs = 100)
{
    const uint32_t start = millis();
    uint8_t status = 0;
    do {
        if (!diagDs2484ReadStatus_(addr, status)) return false;
...
bool diagDs2484OneWireReset_(uint8_t addr)
{
    if (!diagDs2484WaitReady_(addr)) return false;
    const uint8_t cmd = 0xB4;
    if (!diagI2cWrite_(addr, &cmd, 1) || !diagDs2484WaitReady_(addr)) return false;
    uint8_t status = 0;
    return diagDs2484ReadStatus_(addr, status) && ((status & 0x02U) != 0U);
}

bool diagDs2484OneWireWriteByte_(uint8_t addr, uint8_t value)
{
    if (!diagDs2484WaitReady_(addr)) return false;
    const uint8_t cmd[2] = {0xA5, value};
    return diagI2cWrite_(addr, cmd, sizeof(cmd)) && diagDs2484WaitReady_(addr);
}

bool diagDs2484Triplet_(uint8_t addr, bool direction, uint8_t& status)
{
    if (!diagDs2484WaitReady_(addr)) return false;
    const uint8_t cmd[2] = {0x78, (uint8_t)(direction ? 0x80 : 0x00)};
    return diagI2cWrite_(addr, cmd, sizeof(cmd)) && diagDs2484WaitReady_(addr) && diagDs2484ReadStatus_(addr, status);
}

bool diagDs2484SearchAddress_(uint8_t addr, uint8_t index, uint8_t out[8])
{
    uint8_t found = 0;
    ...
    while (!lastDevice) {
        ...
        if (!diagDs2484OneWireReset_(addr)) return false;
        if (!diagDs2484OneWireWriteByte_(addr, 0xF0)) return false;
        ...
        for (uint8_t bitNumber = 1; bitNumber <= 64; ++bitNumber) {
            ...
            if (bitNumber < lastDiscrepancy) {
                direction = (prevRom[byteIndex] & bitMask) != 0;
            } else {
                direction = (bitNumber == lastDiscrepancy);
            }

            uint8_t status = 0;
            if (!diagDs2484Triplet_(addr, direction, status)) return false;
            const bool sbr = (status & 0x20U) != 0U;
            const bool tsb = (status & 0x40U) != 0U;
            const bool dir = (status & 0x80U) != 0U;
            if (sbr && tsb) return false;
            if (!sbr && !tsb && !dir) nextDiscrepancy = bitNumber;
            if (dir) rom[byteIndex] |= bitMask;
        }

        if (diagDsCrc8_(rom, 7) != rom[7]) return false;
        if (found == index) {
            memcpy(out, rom, 8);
            return true;
        }
...
bool constantTimeEquals_(const char* expected, const String& supplied)
{
...
void addSecurityHeaders_(AsyncWebServerResponse* response)
{
    if (!response) return;
...
{
    cfgStore_ = &cfg;
    cfg.registerVar(webAdminUserVar_);
    cfg.registerVar(webAdminPassVar_);
...
{
    if (webSecurity_.user[0] == '\0' && !cfg.set(webAdminUserVar_, FLOW_WEB_ADMIN_USER)) {
        LOGE("Web security: failed to persist administrator user");
        return;
    }

    if (webSecurity_.pass[0] == '\0') {
        const char* configuredPassword = FLOW_WEB_ADMIN_PASSWORD;
        char generatedPassword[25] = {0};
        if (!configuredPassword || configuredPassword[0] == '\0') {
            const uint32_t r0 = esp_random();
            const uint32_t r1 = esp_random();
            const uint32_t r2 = esp_random();
            snprintf(generatedPassword,
        ...
        if (!cfg.set(webAdminPassVar_, configuredPassword)) {
            LOGE("Web security: failed to persist administrator password");
            return;
        }
        Serial.printf("\r\n[SECURITY] Flow.io web credentials: user=%s password=%s\r\n",
                      webSecurity_.user,
                      webSecurity_.pass);
        Serial.println("[SECURITY] Store this password now; it will not be shown by the web API.");
    }

    webCredentialsReady_ = webSecurity_.user[0] != '\0' && webSecurity_.pass[0] != '\0';
    if (!webCredentialsReady_) {
        LOGE("Web security: credentials unavailable; protected HTTP routes will remain locked");
    }
}
...
void WebInterfaceModule::startServer_()
{
    ...
    server_.addMiddleware([](AsyncWebServerRequest* request, ArMiddlewareNext next) {
        next();
        if (request) addSecurityHeaders_(request->getResponse());
    });

    server_.addMiddleware([this](AsyncWebServerRequest* request, ArMiddlewareNext next) {
        const bool provisioningAllowed = allowUnauthenticatedProvisioning_(request);
        if (!provisioningAllowed) {
            ...
            if (!webRequestAuthorized_(request)) {
                // Do not count the browser's initial Digest challenge as a failure.
                // Only a supplied but invalid Authorization header consumes budget.
    ...
    auto beginSpiffsAssetResponse =
        ...
        const bool clientAcceptsGzip =
            acceptEncoding && acceptEncoding->value().indexOf("gzip") >= 0;
        if (clientAcceptsGzip && gzipOverridePath && gzipOverridePath[0] != '\0') {
    ...
    server_.on("/api/wifi/config", HTTP_GET, [this](AsyncWebServerRequest* request) {
        ...
        const bool passConfigured = pass && pass[0] != '\0';
        ...
        const int n = snprintf(out,
                               ...
                               "\"pass\":\"\",\"pass_configured\":%s}",
                               enabled ? "true" : "false",
                               ssidSafe,
                               passConfigured ? "true" : "false");
        if (n <= 0 || (size_t)n >= sizeof(out)) {
            request->send(500, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"Failed\",\"where\":\"wifi.config.get\"}}");
            return;
    ...
    server_.on("/api/wifi/ap", HTTP_GET, [this](AsyncWebServerRequest* request) {
        ...
        const int n = snprintf(out,
                               ...
                               "\"pass\":\"\",\"pass_configured\":true,\"ip\":\"%s\",\"clients\":%u}",
    ...
    server_.on("/api/mqtt/config", HTTP_GET, [this](AsyncWebServerRequest* request) {
        ...
        const int n = snprintf(out,
                               ...
                               "\"user\":\"%s\",\"pass\":\"\",\"pass_configured\":%s,\"baseTopic\":\"%s\","
    ...
    server_.on("/api/wifi/config", HTTP_POST, [this](AsyncWebServerRequest* request) {
        ...
        copyRequestParamValue_(request, "clear_pass", true, clearPassStr, sizeof(clearPassStr), "0");
        ...
        wifi["ssid"] = ssid;
        if (pass[0] != '\0' || clearPass) {
            wifi["pass"] = clearPass ? "" : pass;
        }
    ...
    server_.on("/api/mqtt/config", HTTP_POST, [this](AsyncWebServerRequest* request) {
        ...
        copyRequestParamValue_(request, "port", true, portStr, sizeof(portStr), "8883");
    ...
    server_.on("/api/runtime/manifest", HTTP_GET, [this, beginSpiffsAssetResponse, sendPreparedAssetResponse](AsyncWebServerRequest* request) {
        ...
        if (!response) {
            request->send(heapRejected || buildBusy ? 503 : 500,
                          "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"RuntimeManifestUnavailable\"}}");
            return;
        }
        sendPreparedAssetResponse(request, response, &forensicMeta);
#endif
    ...
    server_.on("/api/diagnostics/network", HTTP_GET, [this](AsyncWebServerRequest* request) {
        HttpLatencyScope latency(request, "/api/diagnostics/network");
        const bool ready = dataStore_ ? networkReady(*dataStore_) : false;
        const IpV4 ip = dataStore_ ? networkIp(*dataStore_) : IpV4{{0, 0, 0, 0}};
        const bool wifiConnected = WiFi.status() == WL_CONNECTED;
        const IPAddress wifiIpAddr = WiFi.localIP();

        AsyncResponseStream* response = request->beginResponseStream("application/json");
        addNoCacheHeaders_(response);
        response->print("{\"ok\":true,\"ready\":");
        response->print(ready ? "true" : "false");
        response->print(",\"ip\":\"");
        response->printf("%u.%u.%u.%u", (unsigned)ip.b[0], (unsigned)ip.b[1], (unsigned)ip.b[2], (unsigned)ip.b[3]);
        response->print("\",\"mode\":\"");
        response->print(wifiConnected ? "wifi" : (ready ? "ethernet" : "none"));
        response->print("\",\"wifi_connected\":");
        response->print(wifiConnected ? "true" : "false");
        response->print(",\"wifi_ip\":\"");
        response->print(wifiIpAddr.toString());
        response->print("\"}");
        request->send(response);
    });

    server_.on("/api/diagnostics/i2c-scan", HTTP_GET, [this](AsyncWebServerRequest* request) {
        HttpLatencyScope latency(request, "/api/diagnostics/i2c-scan");
        if (!flowI2cGlobalLock(750U)) {
            request->send(503, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"Busy\",\"where\":\"diagnostics.i2c_scan.lock\"}}");
            return;
        }

        uint8_t found[0x78] = {0};
        uint8_t foundCount = 0;
        for (uint8_t addr = 0x03U; addr <= 0x77U; ++addr) {
            Wire.beginTransmission(addr);
            const uint8_t err = Wire.endTransmission();
            if (err == 0U && foundCount < sizeof(found)) {
        ...
        AsyncResponseStream* response = request->beginResponseStream("application/json");
        addNoCacheHeaders_(response);
        response->print("{\"ok\":true,\"count\":");
        response->print((unsigned)foundCount);
        response->print(",\"devices\":[");
        for (uint8_t i = 0; i < foundCount; ++i) {
            if (i > 0) response->print(',');
            char addrHex[5] = {0};
            snprintf(addrHex, sizeof(addrHex), "0x%02X", (unsigned)found[i]);
            response->print("{\"address\":\"");
            response->print(addrHex);
            response->print("\",\"decimal\":");
            response->print((unsigned)found[i]);
            response->print('}');
        }
        response->print("]}");
        request->send(response);
    ...
    server_.on("/api/diagnostics/ds18b20", HTTP_GET, [this](AsyncWebServerRequest* request) {
        HttpLatencyScope latency(request, "/api/diagnostics/ds18b20");
        const uint8_t bridgeAddr = (uint8_t)FLOW_DS18_DS2484_ADDRESS;
        if (!flowI2cGlobalLock(1000U)) {
            request->send(503, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"Busy\",\"where\":\"diagnostics.ds18.lock\"}}");
            return;
        }

        const uint8_t resetCmd = 0xF0;
        bool bridgeOk = diagI2cWrite_(bridgeAddr, &resetCmd, 1);
        uint8_t status = 0;
        if (bridgeOk) bridgeOk = diagDs2484ReadStatus_(bridgeAddr, status);

        uint8_t roms[4][8] = {};
        uint8_t count = 0;
        if (bridgeOk) {
            for (uint8_t i = 0; i < 4; ++i) {
                if (!diagDs2484SearchAddress_(bridgeAddr, i, roms[i])) break;
                ++count;
            }
        }
        flowI2cGlobalUnlock();
        ...
        size_t waterLen = 0U;
        size_t airLen = 0U;
        const bool waterAssigned = cfgStore_ &&
            cfgStore_->readRuntimeBlob(NvsKeys::Io::DsRomWater, waterRom, sizeof(waterRom), &waterLen) && waterLen == 8U;
        const bool airAssigned = cfgStore_ &&
            cfgStore_->readRuntimeBlob(NvsKeys::Io::DsRomAir, airRom, sizeof(airRom), &airLen) && airLen == 8U;

        AsyncResponseStream* response = request->beginResponseStream("application/json");
        addNoCacheHeaders_(response);
        response->print("{\"ok\":");
        response->print(bridgeOk ? "true" : "false");
        response->print(",\"bridge\":\"0x");
        response->printf("%02X", (unsigned)bridgeAddr);
        response->print("\",\"count\":");
        response->print((unsigned)count);
        response->print(",\"water_rom\":");
        if (waterAssigned) {
            response->print('"');
            diagPrintRom_(*response, waterRom);
            response->print('"');
        } else response->print("null");
        response->print(",\"air_rom\":");
        if (airAssigned) {
            response->print('"');
            diagPrintRom_(*response, airRom);
            response->print('"');
        } else response->print("null");
        response->print(",\"devices\":[");
        for (uint8_t i = 0; i < count; ++i) {
            ...
            for (uint8_t j = 0; j < 8; ++j) {
                ...
                response->printf("%02X", (unsigned)roms[i][j]);
            }
            response->print("\",\"assigned\":\"");
            if (waterAssigned && memcmp(roms[i], waterRom, 8U) == 0) response->print("water");
            else if (airAssigned && memcmp(roms[i], airRom, 8U) == 0) response->print("air");
            response->print("\"}");
        }
        response->print("]}");
        request->send(response);
    ...
    server_.on("/api/diagnostics/ds18b20-assign", HTTP_POST, [this](AsyncWebServerRequest* request) {
        HttpLatencyScope latency(request, "/api/diagnostics/ds18b20-assign");
        if (!cfgStore_) {
            request->send(503, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"NotReady\",\"where\":\"diagnostics.ds18.assign\"}}");
            return;
        }

        char role[12] = {0};
        char romText[32] = {0};
        copyRequestParamValue_(request, "role", true, role, sizeof(role), "");
        copyRequestParamValue_(request, "rom", true, romText, sizeof(romText), "");
        const bool isWater = strcmp(role, "water") == 0;
        const bool isAir = strcmp(role, "air") == 0;
        uint8_t rom[8] = {};
        if ((!isWater && !isAir) || !diagParseRom_(romText, rom)) {
            request->send(400, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"InvalidArg\",\"where\":\"diagnostics.ds18.assign\"}}");
            return;
        }

        uint8_t otherRom[8] = {};
        size_t otherLen = 0U;
        const char* otherKey = isWater ? NvsKeys::Io::DsRomAir : NvsKeys::Io::DsRomWater;
        if (cfgStore_->readRuntimeBlob(otherKey, otherRom, sizeof(otherRom), &otherLen) &&
            otherLen == 8U && memcmp(rom, otherRom, 8U) == 0) {
            request->send(409, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"Conflict\",\"where\":\"diagnostics.ds18.same_rom\"}}");
            return;
        }

        const char* key = isWater ? NvsKeys::Io::DsRomWater : NvsKeys::Io::DsRomAir;
        if (!cfgStore_->writeRuntimeBlob(key, rom, sizeof(rom))) {
            request->send(500, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"Failed\",\"where\":\"diagnostics.ds18.persist\"}}");
            return;
        }
...
#if defined(FLOW_PROFILE_FLOWIOS3)
    server_.on("/api/diagnostics/sensors", HTTP_GET, [this](AsyncWebServerRequest* request) {
        HttpLatencyScope latency(request, "/api/diagnostics/sensors");
        static const char* const names[6] = {"ORP", "pH", "Pression", "Reserve", "Temperature eau", "Temperature air"};
        static const char* const addresses[6] = {"0x48", "0x48", "0x49", "0x49", "0x18", "0x18"};
        static const char* const inputs[6] = {"AIN0-AIN1", "AIN2-AIN3", "AIN0", "AIN1", "1-Wire eau", "1-Wire air"};
        static const char* const units[6] = {"mV", "pH", "bar", "", "C", "C"};

        AsyncResponseStream* response = request->beginResponseStream("application/json");
        addNoCacheHeaders_(response);
        response->print("{\"ok\":true,\"sensors\":[");
        for (uint8_t i = 0; i < 6U; ++i) {
            if (i > 0U) response->print(',');
            const IOEndpointRuntime* ep = dataStore_ ? &dataStore_->data().io.endpoints[i] : nullptr;
            const bool valid = ep && ep->valid && ep->valueType == IO_VALUE_FLOAT;
            const bool sampleValid = ep && ep->analogSampleValid;
            response->print("{\"index\":");
            response->print((unsigned)i);
            response->print(",\"name\":");
            printJsonEscaped_(*response, names[i]);
            response->print(",\"address\":");
            printJsonEscaped_(*response, addresses[i]);
            response->print(",\"input\":");
            printJsonEscaped_(*response, inputs[i]);
            response->print(",\"unit\":");
            printJsonEscaped_(*response, units[i]);
            response->print(",\"valid\":");
            response->print(valid ? "true" : "false");
            response->print(",\"value\":");
            if (valid) response->printf("%.6f", (double)ep->floatValue); else response->print("null");
            response->print(",\"raw_value\":");
            if (sampleValid) response->printf("%.7f", (double)ep->analogInputValue); else response->print("null");
            response->print(",\"raw_adc\":");
            if (sampleValid && ep->analogRawBinaryValid) response->print((int)ep->analogRawBinary); else response->print("null");
            response->print(",\"age_ms\":");
            if (sampleValid) response->print((unsigned long)(millis() - ep->analogSampleTimestampMs)); else response->print("null");
            response->print('}');
        }
        response->print("]}");
        request->send(response);
    ...
    server_.on("/api/diagnostics/relays", HTTP_GET, [this](AsyncWebServerRequest* request) {
        HttpLatencyScope latency(request, "/api/diagnostics/relays");
        AsyncResponseStream* response = request->beginResponseStream("application/json");
        addNoCacheHeaders_(response);
        response->print("{\"ok\":true,\"relays\":[");
        for (uint8_t slot = 0; slot < POOL_DEVICE_MAX; ++slot) {
            if (slot > 0U) response->print(',');
            PoolDeviceRuntimeStateEntry state{};
            const bool valid = dataStore_ && poolDeviceRuntimeState(*dataStore_, slot, state);
            response->print("{\"slot\":");
            response->print((unsigned)slot);
            response->print(",\"valid\":");
            response->print(valid ? "true" : "false");
            response->print(",\"enabled\":");
            response->print(valid && state.enabled ? "true" : "false");
            response->print(",\"desired_on\":");
            response->print(valid && state.desiredOn ? "true" : "false");
            response->print(",\"actual_on\":");
            response->print(valid && state.actualOn ? "true" : "false");
            response->print(",\"block_reason\":");
            response->print(valid ? (unsigned)state.blockReason : 0U);
            response->print(",\"age_ms\":");
            if (valid) response->print((unsigned long)(millis() - state.tsMs)); else response->print("null");
            response->print('}');
        }
        response->print("]}");
        request->send(response);
    ...
    server_.on("/api/diagnostics/relay", HTTP_POST, [this](AsyncWebServerRequest* request) {
        HttpLatencyScope latency(request, "/api/diagnostics/relay");
        if (!cmdSvc_ && services_) {
            cmdSvc_ = services_->get<CommandService>(ServiceId::Command);
        }
        if (!cmdSvc_ || !cmdSvc_->execute) {
            request->send(503, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"NotReady\",\"where\":\"diagnostics.relay\"}}");
            return;
        }

        char slotStr[8] = {0};
        char valueStr[12] = {0};
        char durationStr[12] = {0};
        copyRequestParamValue_(request, "slot", true, slotStr, sizeof(slotStr), "");
        copyRequestParamValue_(request, "value", true, valueStr, sizeof(valueStr), "0");
        copyRequestParamValue_(request, "duration_ms", true, durationStr, sizeof(durationStr), "0");

        char* end = nullptr;
        const long slotRaw = strtol(slotStr, &end, 10);
        if (slotStr[0] == '\0' || (end && *end != '\0') || slotRaw < 0 || slotRaw > 7) {
            request->send(400, "application/json",
                          "{\"ok\":false,\"err\":{\"code\":\"InvalidArg\",\"where\":\"diagnostics.relay.slot\"}}");
            return;
        }

        const bool active = strcmp(valueStr, "1") == 0
            || strcasecmp(valueStr, "true") == 0
            || strcasecmp(valueStr, "on") == 0;
        long durationRaw = strtol(durationStr, nullptr, 10);
        if (durationRaw < 0) durationRaw = 0;
        if (durationRaw > 30000) durationRaw = 30000;
        ...
        if (!ok) {
            request->send(500,
                          ...
                          (reply[0] != '\0')
                              ? reply
                              : "{\"ok\":false,\"err\":{\"code\":\"Failed\",\"where\":\"diagnostics.relay.exec\"}}");
            return;
        }
        if (active && durationRaw > 0) {
            DiagnosticRelayOffTask* task = static_cast<DiagnosticRelayOffTask*>(pvPortMalloc(sizeof(DiagnosticRelayOffTask)));
            if (!task) {
                char offArgs[64] = {0};
                char offReply[96] = {0};
                snprintf(offArgs, sizeof(offArgs), "{\"slot\":%ld,\"value\":false}", slotRaw);
                (void)cmdSvc_->execute(cmdSvc_->ctx, "pooldevice.write", offArgs, nullptr, offReply, sizeof(offReply));
                request->send(500, "application/json",
                              "{\"ok\":false,\"err\":{\"code\":\"NoMemory\",\"where\":\"diagnostics.relay.timer\"}}");
                return;
            }
            task->cmd = cmdSvc_;
            task->slot = (uint8_t)slotRaw;
            task->delayMs = (uint32_t)durationRaw;
            if (xTaskCreate(diagnosticRelayOffTask_, "diagRelayOff", 3072, task, 1, nullptr) != pdPASS) {
                vPortFree(task);
                char offArgs[64] = {0};
                char offReply[96] = {0};
                snprintf(offArgs, sizeof(offArgs), "{\"slot\":%ld,\"value\":false}", slotRaw);
                (void)cmdSvc_->execute(cmdSvc_->ctx, "pooldevice.write", offArgs, nullptr, offReply, sizeof(offReply));
                request->send(500, "application/json",
                              "{\"ok\":false,\"err\":{\"code\":\"Failed\",\"where\":\"diagnostics.relay.timer\"}}");
                return;
            }
        }
    ...
    server_.on("/api/fwupdate/local/firmware",
               HTTP_POST,
               [this](AsyncWebServerRequest* request) {
                   HttpLatencyScope latency(request, "/api/fwupdate/local/firmware");
                   handleLocalUpdateComplete_(request, false);
               },
               [this](AsyncWebServerRequest* request,
                      String filename,
                      size_t index,
                      ...
                      size_t len,
                      bool final) {
                   handleLocalUpdateUpload_(request, filename, index, data, len, final, false);
               });

    server_.on("/api/fwupdate/local/spiffs",
               HTTP_POST,
               [this](AsyncWebServerRequest* request) {
                   HttpLatencyScope latency(request, "/api/fwupdate/local/spiffs");
                   handleLocalUpdateComplete_(request, true);
               },
               [this](AsyncWebServerRequest* request,
                      String filename,
                      size_t index,
                      ...
                      size_t len,
                      bool final) {
                   handleLocalUpdateUpload_(request, filename, index, data, len, final, true);
               });
    ...
    if (!provisioningOnly_) {
        wsLog_.handleHandshake([this](AsyncWebServerRequest* request) {
            return websocketHandshakeAllowed_(request);
        });
...
void WebInterfaceModule::handleLocalUpdateUpload_(AsyncWebServerRequest* request,
...
{
    if (!request) return;
    // Upload callbacks run while the multipart body is parsed, before the
    // normal middleware/final-handler chain in recent ESPAsyncWebServer.
    if (!webRequestAuthorized_(request) || !csrfRequestAllowed_(request)) return;

    if (index == 0U) {
        localUpdateActive_ = true;
        localUpdateSuccess_ = false;
        localUpdateSpiffs_ = spiffsTarget;
        localUpdateBytes_ = 0U;
        localUpdateError_[0] = '\0';
        localUpdateSignature_[0] = '\0';
        if (localUpdateSha256Active_) {
            mbedtls_sha256_free(&localUpdateSha256_);
            localUpdateSha256Active_ = false;
        }

        if (request->hasHeader("X-Flow-Signature")) {
            snprintf(localUpdateSignature_,
                     sizeof(localUpdateSignature_),
                     "%s",
                     request->header("X-Flow-Signature").c_str());
        }
#if !FLOW_ALLOW_UNSIGNED_UPDATES
        if (OtaTrust::PublicKeyPem[0] == '\0') {
            snprintf(localUpdateError_, sizeof(localUpdateError_), "Cle publique OTA non provisionnee");
            localUpdateActive_ = false;
            return;
        }
        if (localUpdateSignature_[0] == '\0') {
            snprintf(localUpdateError_, sizeof(localUpdateError_), "Signature OTA manquante");
            localUpdateActive_ = false;
            return;
        }
#endif
        ...
        if (!lowerName.endsWith(".bin")) {
            snprintf(localUpdateError_, sizeof(localUpdateError_), "Le fichier doit avoir l extension .bin");
            localUpdateActive_ = false;
            return;
        }
        if (!spiffsTarget && (len == 0U || data == nullptr || data[0] != 0xE9U)) {
            snprintf(localUpdateError_, sizeof(localUpdateError_), "Image firmware ESP32 invalide");
            localUpdateActive_ = false;
            return;
        }
        if (spiffsTarget && len > 0U && data != nullptr && data[0] == 0xE9U) {
            snprintf(localUpdateError_, sizeof(localUpdateError_), "Ce fichier ressemble a un firmware, pas a une image SPIFFS");
            localUpdateActive_ = false;
            return;
        }

        const int command = spiffsTarget ? U_SPIFFS : U_FLASH;
        if (!Update.begin(UPDATE_SIZE_UNKNOWN, command)) {
            snprintf(localUpdateError_, sizeof(localUpdateError_), "Initialisation flash impossible: %s", Update.errorString());
            localUpdateActive_ = false;
            return;
        }
        mbedtls_sha256_init(&localUpdateSha256_);
        if (mbedtls_sha256_starts_ret(&localUpdateSha256_, 0) != 0) {
            snprintf(localUpdateError_, sizeof(localUpdateError_), "Initialisation SHA-256 impossible");
            Update.abort();
            localUpdateActive_ = false;
            return;
        }
        localUpdateSha256Active_ = true;
    }

    if (localUpdateError_[0] == '\0' && len > 0U) {
        if (!localUpdateSha256Active_ ||
            mbedtls_sha256_update_ret(&localUpdateSha256_, data, len) != 0) {
            snprintf(localUpdateError_, sizeof(localUpdateError_), "Calcul SHA-256 impossible");
            Update.abort();
        }
    }
    ...
    if (final) {
        if (localUpdateError_[0] == '\0') {
            uint8_t digest[32] = {0};
            const bool digestOk = localUpdateSha256Active_ &&
                mbedtls_sha256_finish_ret(&localUpdateSha256_, digest) == 0;
            mbedtls_sha256_free(&localUpdateSha256_);
            localUpdateSha256Active_ = false;

            const bool signatureRequired = FLOW_ALLOW_UNSIGNED_UPDATES == 0 ||
                                           localUpdateSignature_[0] != '\0';
            if (!digestOk) {
                snprintf(localUpdateError_, sizeof(localUpdateError_), "Finalisation SHA-256 impossible");
                Update.abort();
            } else if (signatureRequired && !verifyOtaSignature_(digest, localUpdateSignature_)) {
                snprintf(localUpdateError_, sizeof(localUpdateError_), "Signature OTA invalide");
                Update.abort();
                LOGW("local firmware upload rejected: invalid ECDSA signature");
            } else if (!Update.end(true)) {
                snprintf(localUpdateError_, sizeof(localUpdateError_), "Validation flash impossible: %s", Update.errorString());
            } else {
                localUpdateSuccess_ = true;
            }
        }
        if (localUpdateSha256Active_) {
            mbedtls_sha256_free(&localUpdateSha256_);
...
void WebInterfaceModule::handleLocalUpdateComplete_(AsyncWebServerRequest* request, bool spiffsTarget)
{
    if (!request) return;
    if (localUpdateActive_) {
        request->send(409,
                      "application/json",
                      "{\"ok\":false,\"err\":{\"code\":\"UploadIncomplete\",\"msg\":\"Televersement incomplet\"}}");
        return;
    }
    if (localUpdateSpiffs_ != spiffsTarget || !localUpdateSuccess_) {
        sanitizeJsonString_(localUpdateError_);
        char out[256] = {0};
        snprintf(out,
                 sizeof(out),
                 "{\"ok\":false,\"err\":{\"code\":\"LocalUpdateFailed\",\"msg\":\"%s\"}}",
                 localUpdateError_[0] ? localUpdateError_ : "Echec de la mise a jour locale");
    ...
    snprintf(out,
             sizeof(out),
             "{\"ok\":true,\"target\":\"%s\",\"bytes\":%lu,\"reboot_scheduled\":true}",
             spiffsTarget ? "spiffs" : "firmware",
             (unsigned long)localUpdateBytes_);
    request->send(200, "application/json", out);
    scheduleReboot_(1800U, spiffsTarget ? "local-spiffs" : "local-fw");
}
...
{
    if (!isMutatingRequest_(request)) return true;
    if (csrfToken_[0] == '\0' || !requestOriginAllowed_(request, false)) return false;
    ...
    if (!request->hasHeader("X-Flow-CSRF")) return false;
    return constantTimeEquals_(csrfToken_, request->header("X-Flow-CSRF"));
}

bool WebInterfaceModule::websocketHandshakeAllowed_(AsyncWebServerRequest* request) const
{
    return requestOriginAllowed_(request, true);
}
                                             ...
                                             uint32_t& retryAfterSeconds)
{
    ...
    for (uint8_t i = 0; i < kWebAuthThrottleSlots; ++i) {
        WebAuthThrottleEntry& entry = webAuthThrottle_[i];
        if (!entry.used || entry.ip != ip) continue;
        entry.lastSeenMs = now;
        if (entry.blockedUntilMs != 0U && (int32_t)(entry.blockedUntilMs - now) > 0) {
            const uint32_t remainingMs = entry.blockedUntilMs - now;
            retryAfterSeconds = (remainingMs + 999U) / 1000U;
            limited = true;
...
void WebInterfaceModule::noteWebAuthFailure_(AsyncWebServerRequest* request)
{
    ...
    for (uint8_t i = 0; i < kWebAuthThrottleSlots; ++i) {
        ...
        if ((int32_t)(entry.lastSeenMs - webAuthThrottle_[(uint8_t)oldest].lastSeenMs) < 0) {
            oldest = (int8_t)i;
        }
    }
    if (selected < 0) selected = oldest;
    WebAuthThrottleEntry& entry = webAuthThrottle_[(uint8_t)selected];
    if (!entry.used || entry.ip != ip) {
        entry = WebAuthThrottleEntry{};
        entry.used = true;
        entry.ip = ip;
        entry.windowStartMs = now;
    } else if ((now - entry.windowStartMs) >= kWebAuthWindowMs) {
        entry.windowStartMs = now;
        entry.failures = 0U;
    }
    entry.lastSeenMs = now;
    ...
    if (entry.failures >= kWebAuthMaxFailures &&
        (entry.blockedUntilMs == 0U || (int32_t)(entry.blockedUntilMs - now) <= 0)) {
        entry.blockedUntilMs = now + kWebAuthBlockMs;
        newlyBlocked = true;
    }
    portEXIT_CRITICAL(&webAuthThrottleMux_);

    LOGW("Web auth failed ip=%s failures=%u blocked=%u",
...
bool WebInterfaceModule::allowUnauthenticatedProvisioning_(AsyncWebServerRequest* request) const
{
    ...
    if (method == HTTP_GET) {
        ...
        return url.startsWith("/webinterface/") && url != "/webinterface/health";
    }
    if (method == HTTP_POST) {
