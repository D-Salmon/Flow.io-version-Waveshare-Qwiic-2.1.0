...
#ifndef FLOW_HMI_ALLOW_EMPTY_TOKEN
#define FLOW_HMI_ALLOW_EMPTY_TOKEN 0
#endif
    ...
    while (packetSize > 0) {
        if (packetSize <= (int)sizeof(rxBuf_)) {
            ...
            if (len > 0 && hmiUdpValidatePacket(rxBuf_, (size_t)len, header, payload)) {
                ...
                const uint16_t senderPort = udp_.remotePort();
                const HmiUdpMsgType type = (HmiUdpMsgType)header->type;
                if (type == HmiUdpMsgType::Hello) {
                    const auto* hello =
                        (header->len == sizeof(HmiUdpHelloPayload) && payload)
                            ? reinterpret_cast<const HmiUdpHelloPayload*>(payload)
                            : nullptr;
                    if (sessionAuthenticated_ &&
                        (!hello || !tokenAccepted_(hello->tokenCrc))) {
                        LOGW("HMI UDP Hello rejected without replacing active session");
                    ...
                    sessionAuthenticated_ = false;
                } else if (!sessionAuthenticated_ ||
                           senderIp != remoteIp_ ||
                           senderPort != remotePort_) {
                    LOGW("HMI UDP packet rejected: unauthenticated peer type=%u",
...
{
    if (cfgData_.token[0] == '\0') {
#if FLOW_HMI_ALLOW_EMPTY_TOKEN
        return true;
#else
        return false;
#endif
