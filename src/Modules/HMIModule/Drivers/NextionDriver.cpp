...
static constexpr uint8_t HOME_ACTION_CHLORINE_GENERATOR_TOGGLE = 17;
...
static constexpr const char* PressureText = "globals.vaPressure";
static constexpr const char* PressureHomeText = "tPressure";
...
static constexpr const char* ChlorineGeneratorButton = "btElectro";
...
const char* NextionDriver::homeTextObjectName_(HmiHomeTextField field) const
{
    switch (field) {
        ...
        case HmiHomeTextField::Pressure:
            return NextionObject::PressureText;
...
bool NextionDriver::publishHomeText(HmiHomeTextField field, const char* text)
{
    ...
    if (sleeping_) return false;
    const bool sent = sendText_(homeTextObjectName_(field), text);
    if (field != HmiHomeTextField::Pressure) return sent;

    // Home publications only run while pageHome is visible, so the local
    // component can be addressed directly in addition to the global cache.
    return sendText_(NextionObject::PressureHomeText, text) && sent;
}
...
bool NextionDriver::publishHomeStateBits(uint32_t stateBits)
{
    if (!started_) return false;
    if (sleeping_) return false;
    bool ok = sendNum_(NextionObject::StateBits, stateBits);
    const uint32_t chlorineGeneratorOn =
        (stateBits >> HMI_HOME_STATE_CHLORINE_GENERATOR_ON) & 1UL;
    ok = sendNum_(NextionObject::ChlorineGeneratorButton, chlorineGeneratorOn) && ok;
    return ok;
}
        ...
        case NEXTION_CMD_HOME_ACTION:
            ...
            switch (payload[0]) {
                ...
                case HOME_ACTION_DISPLAY_WIFI_FACTORY_RESET:
                    ...
                    return true;
                case HOME_ACTION_CHLORINE_GENERATOR_TOGGLE:
                    out.command = HmiCommandId::HomeChlorineGeneratorToggle;
                    return true;
                default:
                    return false;
            }
