...
static constexpr size_t kPoolLogicDeviceJsonBufSize = 256U;
...
static constexpr uint8_t kNextionAlarmPageAlias = 7U;
...
void HMIModule::refreshHomeBindings_()
{
    if (cfgSvc_ && cfgSvc_->toJsonModule) {
        ...
        bool foundChlorineGeneratorSlot = false;
        ...
        if (cfgSvc_->toJsonModule(cfgSvc_->ctx,
            ...
            slot = chlorineGeneratorDeviceSlot_;
            foundChlorineGeneratorSlot = findJsonUInt16_(jsonBuf, "swg_slot", slot);
            if (foundChlorineGeneratorSlot) {
                chlorineGeneratorDeviceSlot_ = (uint8_t)slot;
            }
            ...
            LOGD("HMI poollogic cfg sensors_trunc=%u device_trunc=%u keys lvl=%u ph=%u orp=%u psi=%u wat=%u air=%u filtr=%u swg=%u php=%u orpp=%u robot=%u fill=%u",
                 ...
                 foundChlorineGeneratorSlot ? 1U : 0U,
...
uint32_t HMIModule::buildHomeStateBits_() const
{
    ...
    if (dsSvc_ && dsSvc_->store) {
        ...
        if (poolDeviceRuntimeState(*dsSvc_->store, chlorineGeneratorDeviceSlot_, state) && state.actualOn) {
            bits |= (1UL << HMI_HOME_STATE_CHLORINE_GENERATOR_ON);
        }
...
uint32_t HMIModule::buildHomeAlarmBits_() const
{
    ...
    if (isAlarmActive_(AlarmId::PoolFiltrationContactorMismatch)) {
        bits |= (1UL << HMI_HOME_ALARM_FILTRATION_CONTACTOR);
    }
    if (isAlarmActive_(AlarmId::PoolChlorineGeneratorContactorMismatch)) {
        bits |= (1UL << HMI_HOME_ALARM_CHLORINE_CONTACTOR);
    }
    return bits;
}
...
bool HMIModule::publishHomeText_(HmiHomeTextField field)
{
    ...
    switch (field) {
        ...
        case HmiHomeTextField::Orp:
            ...
            break;
        case HmiHomeTextField::Pressure:
            runtimeIndex = psiRuntimeIndex_;
            break;
        default:
            return false;
    }
    ...
    switch (field) {
        ...
        case HmiHomeTextField::Orp:
            ...
            break;
        case HmiHomeTextField::Pressure:
            if (hasValue) snprintf(value, sizeof(value), "%.2f bar", (double)rawValue);
            else snprintf(value, sizeof(value), "--.-- bar");
            break;
        default:
            return false;
    }
...
void HMIModule::flushHomePublish_()
{
    ...
    if ((pending & kHomePublishPsi) != 0U && publishHomeText_(HmiHomeTextField::Pressure)) {
    ...
    if (e.id == EventId::ConfigChanged && e.payload && e.len >= sizeof(ConfigChangedPayload)) {
        ...
        if (p->id == DATAKEY_NETWORK_READY || p->id == DATAKEY_NETWORK_IP || p->id == DATAKEY_MQTT_READY) {
            ...
            homePublishMask |= kHomePublishPsi;
                   ...
                   p->id == (DataKey)(DATAKEY_POOL_DEVICE_STATE_BASE + chlorineGeneratorDeviceSlot_)) {
            homePublishMask |= kHomePublishStateBits;
        }
        ...
        if (id == AlarmId::PoolPhTankLow ||
            ...
            id == AlarmId::PoolFiltrationContactorMismatch ||
            id == AlarmId::PoolChlorineGeneratorContactorMismatch) {
            ledDirty = true;
            homePublishMask |= kHomePublishAlarmBits;
        }
    }
...
bool HMIModule::executeHmiCommand_(HmiCommandId command, uint8_t value)
{
    ...
    switch (command) {
        ...
        case HmiCommandId::HomeChlorineGeneratorToggle:
            if (!readPoolDeviceActualOn_(chlorineGeneratorDeviceSlot_, current)) {
                setHomeErrorMessage_("ReadStateFailed", true);
                return false;
            }
            return executeCommandBool_("poollogic.chlorine_generator.write", !current);
