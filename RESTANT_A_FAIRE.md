...
La version `2.1.0` est une edition durcie et allegee consacree au controleur
...
1. Ajouter une protection CSRF, une limitation des tentatives et la
   journalisation des echecs d'authentification Web.
2. Restreindre MQTT avec des ACL uniques par appareil et, pour les commandes
   sensibles, une signature avec nonce, timestamp et protection anti-rejeu.
3. Remplacer l'OTA non signee actuellement desactivee par une OTA HTTPS signee,
   avec verification cryptographique, rollback et protection anti-downgrade.
4. Faire evoluer le protocole HMI vers HMAC-SHA256 avec challenge aleatoire,
   compteur et fenetre anti-rejeu.
5. Preparer Secure Boot v2, le chiffrement flash/NVS et une procedure de
   programmation controlee des eFuses pour la fabrication.
6. Signer le manifeste de livraison: les sommes SHA-256 actuelles assurent
...
2. Ajouter des tests d'integration pour l'authentification Web, la rotation des
   identifiants, MQTT TLS, les rejeux HMI et les coupures pendant une mise a
   jour.
...
3. Protection CSRF et durcissement MQTT.
4. OTA signee.
