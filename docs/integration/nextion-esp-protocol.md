...
- `globals.vaPressure.txt`
- `pageHome.tPressure.txt` (composant local, mis a jour directement quand Home est visible)
...
- `globals.vaPressure.txt` : pression issue de l'ADS1115 en bars, exemple `1.42 bar`
...
- bit `5` : alarme PSI
- bit `6` : defaut retour contacteur filtration
- bit `7` : defaut retour contacteur electrolyseur
...
- `0x11` : `CHLORINE_GENERATOR_TOGGLE` (électrolyseur)

Valeur :
- `0x00` : faux / OFF
- `0x01` : vrai / ON

Pour l'action `0x11`, la valeur est ignorée : chaque trame inverse l'état demandé.

Électrolyseur bascule ON/OFF :

```text
printh 23 03 60 11 01
```
...
- `0x0B` : page `pageAlarms`

IDs internes du projet Intelligent 800x480 actuel :
- `globals=0`
- `pageHome=1`
- `pageCfgMenu=2`
- `Splash=3`
