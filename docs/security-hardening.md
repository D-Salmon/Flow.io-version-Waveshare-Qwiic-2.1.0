...
The HMI server accepts non-Hello packets only after a successful token handshake
and only from the authenticated IP/UDP-port pair. Empty HMI tokens are rejected
unless a development build sets `FLOW_HMI_ALLOW_EMPTY_TOKEN=1`.
...
The current HMI protocol still sends only a CRC-derived token and does not
cryptographically authenticate every packet. A future protocol version should
use a nonce plus HMAC (and replay protection) for hostile networks.
